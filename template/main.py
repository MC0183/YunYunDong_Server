...
def parse_args():
    ...
    parser.add_argument('-u', '--username', type=str, help='登录账号')
...
def default_post(router, data, headers=None, m_host=None, isBytes=False, gen_sign=True, key_ctx=None):
    global my_token, my_device_id, my_device_name, my_utc, my_uuid, my_sign, my_app_edition
    if m_host is None:
        m_host = my_host
    url = m_host + router
    ...
    for attempt in range(max_retries):
        try:
            # 每次重试都重新生成时间戳和签名，防止请求过期
            if gen_sign:
                current_utc = str(int(time.time()))
            else:
                current_utc = my_utc
            current_sign = getsign(current_utc, my_uuid) if gen_sign else my_sign
            ...
            req = requests.post(url=url, data=json.dumps(data_json), headers=current_headers, timeout=15)
            ...
            # 【优化】使用引入随机抖动的指数退避策略，错峰重试
            # 等待时间规律大致为：3~6秒 -> 5~8秒 -> 9~12秒 -> 17~20秒
            sleep_time = random.uniform(2.0, 5.0) + (2 ** attempt)
            print(f"网络异常，请求 {router} 失败 (重试 {attempt + 1}/{max_retries}): {e}")
            if attempt < max_retries - 1:
                print(f"    -> 触发防并发机制，等待 {sleep_time:.2f} 秒后错峰重试...")
                time.sleep(sleep_time) 
            else:
                print("达到最大重试次数，抛出异常。")
                raise e

def check_and_update_token(cfg_path):
    global my_token, my_device_id, my_device_name, my_uuid, my_sys_edition
    print("正在检查Token有效性...")
    try:
        # 发送最小请求测试 Token
        resp_text = default_post("/run/getHomeRunInfo", "")
        resp_json = json.loads(resp_text)
        if resp_json.get('code') == 200 and 'data' in resp_json:
            print("Token有效，可以直接跑步！")
            return True
    except Exception as e:
        pass
    
    print("Token已失效或不存在，尝试读取 config.ini 自动重新登录...")
    try:
        # 调用 Login.main() 自动登录
        login_result = Login.main()
        if login_result is not None:
            my_token, my_device_id, my_device_name, my_uuid, my_sys_edition = login_result
            # 静默更新配置文件，不使用 input()
            ...
            config.set('User', 'device_name', my_device_name)
            config.set('User', 'sys_edition', my_sys_edition)
            with open(cfg_path, 'w+', encoding='utf-8') as f:
                config.write(f)
            print("重新登录成功，新Token已保存并生效。")
            return True
    except Exception as e:
        print(f"自动登录失败，请检查账号密码或验证码机制: {e}")
...
class Yun_For_New:
    ...
    def __init__(self, auto_generate_task = False):
        ...
        if 'data' not in resp_json:
            print(f"获取跑步信息失败，这通常意味着账号状态异常。服务器返回: {resp_json}")
            sys.exit(1) # 状态码 1：账号状态异常/Token相关
        data = resp_json['data']['cralist'][0]
    ...
    def finish_by_points_map(self):
        ...
        try:
            ...
            if "不合格" in resp_json.get("msg", ""):
                print("警告：服务端判定跑步成绩不合格，强行中止程序！")
                import sys
                sys.exit(2) # 以错误码 2 退出，触发 Shell 的 [FAILED]
        except Exception:
            pass
    ...
    def finish(self):
        ...
        try:
            ...
            if "不合格" in resp_json.get("msg", ""):
                print("警告：服务端判定跑步成绩不合格，强行中止程序！")
                import sys
                sys.exit(1) # 以错误码 1 退出，触发 Shell 的 [FAILED]
        except Exception:
            pass

def main(run = True):
    # 【新增】启动时增加随机防并发延迟 (1~8秒)
    # 防止 cron 瞬间拉起大量进程造成第一波 WAF 拥堵封禁
    startup_delay = random.uniform(1.0, 8.0)
    ...
    # --- 修改这里：无论是否 auto_run，都进行静默 Token 检查 ---
    if not check_and_update_token(cfg_path):
        print("致命错误：无法获取有效Token，程序终止。")
        sys.exit(1) # 状态码 1：系统资源/Token失效
    # --------------------------------------------------------
    ...
    try:
        if sure == 'y':
            ...
            print("退出。")
            sys.exit(0) # 正常退出不跑的情况也算成功或忽略

        # 如果前面的流程都顺利走完了，报成功
        print("任务执行完毕，准备退出。")
        sys.exit(0) # 状态码 0：大获全胜

    except Exception as e:
        print("跑步失败了，发生其他未知代码错误：")
        print(e)
        # 注意：这里千万不要加 input()，因为 crontab 后台无人值守，遇到 input 会卡死！
        sys.exit(3) # 状态码 3：其他未捕获的错误
